...
class DevicesResource(BaseResource):
    ...
    def _build_custom_data_result(
        ...
        if isinstance(meta, dict):
            new_meta = dict(meta)
            new_meta["total"] = len(merged_rows)
    ...
    def custom_data(
        ...
        *,
        ...
        Uses iterative backward pagination by `created_at` to return the
        ...
        start_date_utc = _to_utc_datetime(start_date)
        if end_date is None:
            end_date = _utcnow()
        end_date_utc = _to_utc_datetime(end_date)
        if start_date_utc >= end_date_utc:
            raise ValueError("start_date must be earlier than end_date")

        config_id_int: Optional[int] = None
        ...
        return self._build_custom_data_result(last_response, merged_rows)
